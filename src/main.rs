    ...
    match args {
        Cli::Mail => {
            ...
            if let Ok(Some(letters)) = letters {
                ...
                let table = table
                    .table()
                    .title(vec!["Name", "ID", "Status", "Creation Date"])
                    .display()
                    .unwrap();
        ...
        Cli::View { id } => {
            let letter = client.get_letter(id).await;
            if let Ok(Some(letter)) = letter {
                let mut table: Vec<Vec<CellStruct>> = Vec::new();
                if let Some(id) = letter.id {
                    ...
                    if !events.is_empty() {
                        ...
                        for event in events {
                            ...
                            if let Some(happened_at) = event.happened_at {
                                ...
                                table.push(vec!["Happened At".cell(), "no happening date".cell()]);
                            }
